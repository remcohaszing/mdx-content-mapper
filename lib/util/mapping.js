 ...
 * @param {number} length
 * @param {boolean | 'self'} [closing]
 * @returns {string}
 */
export function mapMarkdownToJsx(mappings, generated, tagName, originalStart, length, closing) {
  ...
  mappings.push([
    result.length,
    componentsNamespace.length,
    originalStart,
    length,
    /** @satisfies {SpanMapKind.Atom} */ (1),
    0
  ])
  result += componentsNamespace
  result += '.'
  mappings.push([
    result.length,
    tagName.length,
